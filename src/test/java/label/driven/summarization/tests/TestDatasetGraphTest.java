 ...
 * @since 1.0 6/4/18.
 */
@DisplayName("TestDatasetGraphTest")
class TestDatasetGraphTest {
    ...
    void loadGraph() throws InterruptedException, ExecutionException, SummaryGraphConstructionException, IOException {
        URL schemaFile = getClass().getClassLoader().getResource("test_dataset/schema.json");
        URL edgeFile = getClass().getClassLoader().getResource("test_dataset/edge_file.txt");
    ...
    private void grouping() throws ExecutionException, InterruptedException {
        ...
        assert(graphSession.getListOfLabels().get(0).equals("authors"));

        Map<Integer, String> correspondence = grouping.getCorrespondenceGroupingLabel();
        /*  */
        assert(correspondence.entrySet().stream().filter(p -> p.getValue().equals("authors")).findAny()
        ...
        /* the number of grouping should be `4`*/
        assert(groupings.size() == 4);
    }


    private void evaluation () throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException {
        grouping();
        ...
        assert (summaryEvaluation.getNumVertices() == 39);
        assert (summaryEvaluation.getNumEdges() == 39);

        assert (summaryEvaluation.
                filter(new VertexFilter("vertex.outDegree() == 0 && vertex.inDegree() == 0"))
                .getNumVertices() == 0);
...
//        Util.printGraph(summaryEvaluation);
    ...
    void frequency() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert(summaryFrequency.getNumVertices() == 4);
        assert(summaryFrequency.getNumEdges() == 4);
        ...
        Util.printGraph(summaryFrequency);
...
//
//        assert (summaryClustering.getNumEdges() == 7);
//        assert (Util.sumOfLabelsOccurrences(summaryClustering) == 47);
//        assert (Util.sumOfEdgesWeights(summaryClustering) == 123);
//    }
