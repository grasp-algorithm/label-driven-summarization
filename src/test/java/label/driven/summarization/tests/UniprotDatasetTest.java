 ...
 * @since 1.0 6/10/18.
 */
@DisplayName("UniprotDatasetTest")
class UniprotDatasetTest {
    private static final int NUMBER_EDGES = 1152;
    private GraphSession graphSession;
    private Schema schema;
    private Grouping grouping;
    private PgxGraph summaryEvaluation;
    private PgxGraph summaryFrequency;

    @BeforeEach
    void loadGraph() throws InterruptedException, ExecutionException, SummaryGraphConstructionException, IOException {
        URL schemaFile = getClass().getClassLoader().getResource("uniprot_dataset/schema.json");
        URL edgeFile = getClass().getClassLoader().getResource("uniprot_dataset/edge_file.txt");
        ...
        graphSession.buildGraph(schema, edgeFile.getPath(), null);
    ...
    private void grouping() throws ExecutionException, InterruptedException {
        ...
        assert (graphSession.getListOfLabels().get(0).equals("HasKeyword"));

        Map<Integer, String> correspondence = grouping.getCorrespondenceGroupingLabel();

        /*  */
        assert (correspondence.entrySet().stream().filter(p -> p.getValue().equals("HasKeyword")).findAny()
        ...
        /* the number of groupings should be `6`*/
        assert (groupings.size() == 6);
    }
    ...
    private void evaluation() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        /* The number of vertices without grouping should be 0. */
        long numberOfVerticesWithNoneGrouping = graphSession.getGraph().filter(new VertexFilter(String.format("vertex.%s = %s",
                PrefixPropertyConstant.GROUPING.toString(), Constants.GROUPING_DEFAULT_VALUE))).getNumVertices();
        assert (numberOfVerticesWithNoneGrouping == 0L);
        ...
        assert (summaryEvaluation.getNumVertices() == 91);
        assert (summaryEvaluation.getNumEdges() == 92);
        assert (summaryEvaluation.filter(new VertexFilter("vertex.outDegree() == 0 && " +
                "vertex.inDegree() == 0")).getNumVertices() == 1);
    ...
    void frequency() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert (summaryFrequency.getNumVertices() == 8);
        assert (summaryFrequency.getNumEdges() == 12);
...
//
//        assert (summaryClustering.getNumEdges() == 17);
//        assert (Util.sumOfLabelsOccurrences(summaryClustering) == 343);
//        assert (Util.sumOfEdgesWeights(summaryClustering) == NUMBER_EDGES);
//    }
