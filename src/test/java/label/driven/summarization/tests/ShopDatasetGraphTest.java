 ...
 * @since 1.0 6/4/18.
 */
@DisplayName("ShopDatasetGraphTest")
class ShopDatasetGraphTest {
    ...
    void loadGraph() throws InterruptedException, ExecutionException, SummaryGraphConstructionException, IOException {
        URL schemaFile = getClass().getClassLoader().getResource("shop_dataset/schema.json");
        URL edgeFile = getClass().getClassLoader().getResource("shop_dataset/edge_file.txt");
    ...
    private void grouping() throws ExecutionException, InterruptedException {
        ...
        /* The most frequent label is `hasReview`, so it should appear at the head of the list */
        assert (graphSession.getListOfLabels().get(0).equals("hasReview"));

        Map<Integer, String> correspondence = grouping.getCorrespondenceGroupingLabel();

        /*  */
        assert (correspondence.entrySet().stream().filter(p -> p.getValue().equals("hasReview")).findAny()
        ...
        /* the number of groupings should be `63`*/
        assert (groupings.size() == 63);
    }
    ...
    private void evaluation() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert (summaryEvaluation.getNumVertices() == 2683);
        assert (summaryEvaluation.getNumEdges() == 4061);
        assert (summaryEvaluation.filter(new VertexFilter("vertex.outDegree() == 0 && " +
                "vertex.inDegree() == 0")).getNumVertices() == 63);
    ...
    void frequency() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert (summaryFrequency.getNumVertices() == 426);
        assert (summaryFrequency.getNumEdges() == 2276);
...
//
//        System.out.println(Util.sumOfEdgesWeights(summaryClustering));
//        System.out.println(Util.sumOfLabelsOccurrences(summaryClustering));
//        assert (summaryClustering.getNumEdges() == 259);
//        assert (Util.sumOfEdgesWeights(summaryClustering) == NUMBER_EDGES);
//        assert (Util.sumOfLabelsOccurrences(summaryClustering) == 1049);
//
//
