...
class ToyTest {
    ...
    void loadGraph() throws IOException {
        ...
        URL config = getClass().getClassLoader().getResource("toy_dataset/toy-graph-config.json");
        URL schemaFile = getClass().getClassLoader().getResource("toy_dataset/schema_toy_graph.json");

                graphSession = new GraphSession();
        graphSession.initialize();
        graphSession.buildGraph(config.getPath());
    ...
    private void grouping() throws ExecutionException, InterruptedException {
        ...
        assert (graphSession.getListOfLabels().get(0).equals("l0"));

        Map<Integer, String> correspondence = grouping.getCorrespondenceGroupingLabel();
        /*  */
        assert (correspondence.entrySet().stream().filter(p -> p.getValue().equals("l0")).findAny()
        ...
        /* the number of groupings should be `3`*/
    ...
    private void evaluation() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert (summaryEvaluation.getNumVertices() == 7);
        assert (summaryEvaluation.getNumEdges() == 10);
    ...
    void frequency() throws ExecutionException, InterruptedException, SummaryGraphConstructionException, MergeStrategyException, PgqlException {
        ...
        assert (summaryFrequency.getNumVertices() == 2);
        assert (summaryFrequency.getNumEdges() == 2);
...
//
//        assert (summaryClustering.getNumEdges() == 3);
//
//        assert (Util.sumOfLabelsOccurrences(summaryClustering) == 16);
//        assert (Util.sumOfEdgesWeights(summaryClustering) == 30);
//
//    }
